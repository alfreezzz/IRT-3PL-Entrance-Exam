    ...
    <form action="{{ route('exams.questions.store', $exam) }}" method="POST" class="space-y-6"
        ...
        <div @change="questionType = $event.target.value">
            <x-form.select
                ...
                option-value="id"
                option-label="label"
                :value="old('question_type', 'single_choice')"
...
document.addEventListener('alpine:init', () => {
    Alpine.data('questionForm', () => ({
        questionType: '{{ old('question_type', 'single_choice') }}',
        statements: @json(old('statements', [])),
        
        init() {
            // Initialize with at least 3 empty statements if none exist
