        ...
        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-700/50 dark:bg-slate-900">
            ...
            <div class="mt-6 space-y-4">
                @foreach($subtests as $subtest)
                    @php
                        $weight = old('weights.' . $subtest->id, optional($program->programSubtestWeights->firstWhere('subtest_id', $subtest->id))->weight ?? 0);
                    @endphp
...
function programEditForm() {
    return {
        portfolioRequired: @json((bool) old('portfolio_required', $program->portfolio_required)),
        portfolioWeight: @json((float) old('portfolio_weight', $program->portfolio_weight ?? 0)),
        weights: @json(collect($subtests)->mapWithKeys(function($s) use ($program) {
            $existing = optional($program->programSubtestWeights->firstWhere('subtest_id', $s->id))->weight ?? 0;
            return [$s->id => (float) old('weights.' . $s->id, $existing)];
        })),
